...
用法:
  python3 scaffold_leaf.py operators/triton/vector-add
  python3 scaffold_leaf.py fundamentals/pytorch-tensor-lifecycle/view-vs-reshape --force
  python3 scaffold_leaf.py --list-domain operators/triton   # 预览该领域会生成什么
"""
...
def readme_md(rel: str, code_kind: str) -> str:
    ...
    return (
        ...
        "若该叶子已预置 AI 编写的 `tutorial.md`，学习流程为：教程 → 写下预测 → 实践(跑代码验证) → 填 `note.md` → 闭卷作答 `questions.md`。\n"
        "脚手架不生成教程；教程由课程作者按优先级显式写入。文件所有权见 `atlas/learning-artifact-contract.md`。\n\n"
...
RUN_SH = {
    ...
    "bench": "#!/usr/bin/env bash\nset -euo pipefail\nmkdir -p results\npython3 bench.py | tee results/run.log\n",
}
